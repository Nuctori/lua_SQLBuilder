...
describe("utils.Make_JsonQuery", function()
  it("returns {sql, param} pairs for scalars with string params", function()
    local dialect = require "lua_SQLBuilder.dialect".resolve("mysql")
    local out = utils.Make_JsonQuery("profile", { star = 5 }, dialect)
    assert.equal(1, #out)
  ...
  it("handles nested paths", function()
    local dialect = require "lua_SQLBuilder.dialect".resolve("mysql")
    local out = utils.Make_JsonQuery("profile", { nested = { k = "x" } }, dialect)
