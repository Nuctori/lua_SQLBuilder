...
-- (default "sqlite"); credentials come from env, matching the GitHub
-- Actions services (see .github/workflows/ci.yml).
--
--   db.current()            → dialect name
...
--
-- Driver matrix:
--   sqlite    lsqlite3        (in-process, ? binding)
--   postgres  pgmoon          (? → $n conversion, binding)
--   mysql     luasql.mysql    (inline only, LuaSQL 2.x has no binding)
--   duckdb    luasql.duckdb   (in-process, inline only)
--   oracle    luasql.oci8     (Instant Client, inline only)
--   clickhouse HTTP (luasocket) (inline only, FORMAT JSONEachRow)
...
-------------------------------------------------------------------------------
-- MySQL / DuckDB / Oracle (LuaSQL) — inline execution only
-- (LuaSQL 2.x line has no ? binding; fetch 'a' = named rows in these drivers)
-------------------------------------------------------------------------------

local function luas_driver_or(mod)
  local factory = mod.mysql or mod.duckdb or mod.oci8
  if factory then
    return factory()
  end
  return mod
...
-- Shared LuaSQL connection: exec/query/close with named-row normalization.
local function make_luasql_conn(dialect, handle, env_obj)
  local conn = {
    dialect = dialect,
  ...
  function conn:exec(sql, ...)
    local n = select("#", ...)
    if n > 0 then
      return nil, dialect .. " driver (LuaSQL 2.x) does not support bound parameters"
    end
  ...
  function conn:last_insert_id()
    local ok_li, id = pcall(self._conn.getlastautoid, self._conn)
    if ok_li and id then
      return id
    end
    return nil
  end
...
local function connect_luasql_mysql()
  ...
  return make_luasql_conn("mysql", handle, env_obj)
end

local function connect_luasql_duckdb()
  local ok, luasql_driver = pcall(require, "luasql.duckdb")
  if not ok then
    return nil, "luasql.duckdb not available"
  end
  local env_obj = luas_driver_or(luasql_driver)
  -- in-process embedded database
  local handle, connect_err = env_obj:connect(":memory:")
  if not handle then
    env_obj:close()
    return nil, "duckdb open failed: " .. tostring(connect_err)
  end
  return make_luasql_conn("duckdb", handle, env_obj)
end

local function connect_luasql_oci8()
  local ok, luasql_driver = pcall(require, "luasql.oci8")
  if not ok then
    return nil, "luasql.oci8 not available"
  end
  local env_obj = luas_driver_or(luasql_driver)
  local user = env("ORACLE_USER", "app")
  local password = env("ORACLE_PASSWORD", "app")
  local host = env("ORACLE_HOST", "127.0.0.1")
  local port = env("ORACLE_PORT", "1521")
  local service = env("ORACLE_SERVICE", "FREEPDB1")
  -- EZCONNECT: host:port/service
  local handle, connect_err = env_obj:connect(fmt("%s:%s/%s", host, port, service), user, password)
  if not handle then
    env_obj:close()
    return nil, "connection failed: " .. tostring(connect_err)
  end
  return make_luasql_conn("oracle", handle, env_obj)
end

-------------------------------------------------------------------------------
-- ClickHouse (HTTP interface via luasocket) — inline execution only
-------------------------------------------------------------------------------

local function connect_clickhouse()
  local ok_http, http = pcall(require, "socket.http")
  local ok_ltn12, ltn12 = pcall(require, "ltn12")
  local ok_cjson, cjson = pcall(require, "cjson")
  if not (ok_http and ok_ltn12 and ok_cjson) then
    return nil, "clickhouse HTTP needs luasocket + cjson"
  end
  local host = env("CLICKHOUSE_HOST", "127.0.0.1")
  local port = env("CLICKHOUSE_PORT", "8123")
  local user = env("CLICKHOUSE_USER", "default")
  local password = env("CLICKHOUSE_PASSWORD", "clickhouse")
  local url = fmt("http://%s:%s/?user=%s&password=%s", host, port, user, password)
  ...
  local function post(self, sql)
    ...
    local code, headers = self._http.request{
      ...
      sink = self._ltn12.sink.table(resp),
      headers = { ["Content-Type"] = "application/x-www-form-urlencoded" },
    }
    local body = table.concat(resp)
    if code ~= 200 then
      -- ClickHouse returns errors in the body with a non-200 status
      return nil, "clickhouse HTTP " .. tostring(code) .. ": " .. (body or "")
    end
    return body
  end
  ...
  function conn:exec(sql, ...)
    local n = select("#", ...)
    if n > 0 then
      return nil, "clickhouse driver does not support ? binding (use to_sql)"
    end
    local _, err = post(self, sql)
    if err then
      return nil, err
    end
    return true
  ...
  function conn:query(sql)
    -- append JSONEachRow for SELECTs (exec never reaches here)
    local body, err = post(self, sql .. " FORMAT JSONEachRow")
    if err then
      return nil, err
    end
...
function db.connect()
  ...
  elseif dialect == "duckdb" then
    conn, reason = connect_luasql_duckdb()
  elseif dialect == "clickhouse" then
    conn, reason = connect_clickhouse()
  elseif dialect == "oracle" then
    conn, reason = connect_luasql_oci8()
  else
